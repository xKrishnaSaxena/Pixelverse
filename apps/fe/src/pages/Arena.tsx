...
const token =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2N2RkN2FjMDQ4Y2FlMGY4YWQxNTJiNTAiLCJyb2xlIjoiQWRtaW4iLCJpYXQiOjE3NDI2NTA1MTF9.6mguLjSpMLKMXRI3LYeoKB4R_L_OPcuS2DvByuLy15Y";
...
const Arena = () => {
  ...
  useEffect(() => {
    const fetchSpaceDimensions = async () => {
      try {
        const spaceData = await axios.get<SpaceResponse>(
          `http://localhost:3000/api/v1/space/${params.spaceId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        setSpaceDimensions({
          width: parseInt(spaceData.data.dimensions.split("x")[0]),
          height: parseInt(spaceData.data.dimensions.split("x")[1]),
        });
      } catch (error) {
        console.error("Error fetching space dimensions:", error);
      }
    };

    if (params.spaceId) {
      fetchSpaceDimensions();
    }
  }, [params.spaceId]);
  ...
  useEffect(() => {
    ...
    if (canvas && spaceDimensions.width > 0 && spaceDimensions.height > 0) {
      canvas.width = spaceDimensions.width * 50; // Assuming each grid unit is 50px
      canvas.height = spaceDimensions.height * 50;
    }
  }, [spaceDimensions]);
