...
export const Arena = () => {
  ...
  const handleCallUser = useCallback(
    async (recipient: string) => {
      ...
      console.log("Calling user " + recipient);
      ...
      if (peer.peer) {
        for (const track of stream.getTracks()) {
          ...
          console.log("Added track to peer:", track);
        }
      }
      ...
      console.log("Sent call-user");
    },
    [callStatus]
  );

  const handleIncomingCall = useCallback(
    async (offer: any, from: string) => {
      if (callStatus !== "idle" || !wsRef.current) return;
  ...
  const handleNegoNeeded = useCallback(async () => {
    ...
    console.log("Sent peer:negotiation-needed");
  }, [remoteUserId]);
  ...
  const handleNegoNeedIncoming = useCallback(
    async ({ from, offer }: { from: string; offer: any }) => {
      ...
      console.log("Sent peer:nego:done");
    },
    []
  );
  ...
  useEffect(() => {
    ...
    peer.peer.addEventListener("track", (ev) => {
      ...
      console.log("Remote stream set:", ev.streams[0]);
    });
    ...
    console.log("Added negotiationneeded listener");
    return () => {
      if (!peer.peer) return;
  ...
  const handleWebSocketMessage = useCallback(
    (message: any) => {
      ...
      switch (message.type) {
        ...
        case "video-call-incoming":
          handleIncomingCall(message.payload.offer, message.payload.from);
          break;
    ...
    [currentUser, users, activeChatUser, remoteUserId, handleEndCall]
  );
                  ...
                  {Array.from(nearbyUsers).map((userId) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="ml-2 px-3 py-2 text-sm rounded bg-green-500 hover:bg-green-600 disabled:bg-gray-500"
